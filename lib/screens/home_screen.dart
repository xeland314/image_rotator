...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _exportFinal() async {
    ...
    try {
      ...
      await _loadHistory();
      if (mounted) {
        final msg = Platform.isLinux ? 'Imagen guardada en $savedPath (calidad 95)' : 'Imagen guardada en galería (calidad 95)';
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(color: cs.primaryContainer.withValues(alpha: 0.5), borderRadius: BorderRadius.circular(16)),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                  ...
                  child: Text('HERRAMIENTA DE IMÁGENES', style: TextStyle(fontSize: 10, letterSpacing: 1.5, color: cs.primary, fontWeight: FontWeight.bold))),
              const SizedBox(height: 10),
              Text('Rotador de Imágenes Multi-Giro', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
              const SizedBox(height: 6),
              Text('Carga una imagen y aplica múltiples operaciones concatenadas. Cada giro se aplica sobre el anterior (ej. 900°+770°).', style: theme.textTheme.bodyMedium?.copyWith(color: cs.onSurfaceVariant)),
            ]),
          ),
          ...
          if (hasImage) ...[
            ...
            const SizedBox(height: 6),
            Text(_imageName, style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant), textAlign: TextAlign.center),
          ],
          const SizedBox(height: 20),
          // 2. Operaciones
          ...
          if (_ops.isEmpty)
            Container(
                ...
                decoration: BoxDecoration(color: cs.surfaceContainerHigh, borderRadius: BorderRadius.circular(12), border: Border.all(color: cs.outlineVariant)),
                child: Text('No hay operaciones. Agrega una abajo.', style: TextStyle(fontStyle: FontStyle.italic, color: cs.onSurfaceVariant))),
          ..._ops.asMap().entries.map((e) {
            ...
            return Card(
              ...
              child: ListTile(
                dense: true,
                leading: Text('${i + 1}', style: TextStyle(fontFamily: 'monospace', color: cs.onSurfaceVariant)),
              ...
              child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
                ...
                Expanded(
                    child: DropdownButtonFormField<RotationDirection>(
                        ...
                        items: const [
                          DropdownMenuItem(value: RotationDirection.cw, child: Text('Horario', overflow: TextOverflow.ellipsis)),
                          DropdownMenuItem(value: RotationDirection.ccw, child: Text('Antihorario', overflow: TextOverflow.ellipsis)),
                        ],
                        onChanged: (v) {
            ...
            child: Column(children: [
              Material(
                ...
                child: CheckboxListTile(
                    ...
                    title: const Text('Trazar arco (goniómetro con vueltas)', style: TextStyle(fontSize: 13)),
                    dense: true,
              ...
              Material(
                ...
                child: CheckboxListTile(
                    ...
                    title: const Text('Incluir resultado directo (ángulo efectivo)', style: TextStyle(fontSize: 13)),
                    dense: true,
                    controlAffinity: ListTileControlAffinity.leading),
              ),
            ]),
          ...
          Row(children: [
            Expanded(child: FilledButton.icon(onPressed: canApply ? _exportFinal : null, icon: const Icon(Icons.save_alt), label: const Text('Guardar en galería'))),
            const SizedBox(width: 8),
          ...
          if (canApply) ...[
            const Divider(),
            const SizedBox(height: 8),
            Text('Resultado final', style: theme.textTheme.labelSmall?.copyWith(color: cs.primary, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
            const SizedBox(height: 6),
            Text(
            ...
            if (_showTrace) ...[
              ...
              const SizedBox(height: 4),
              Row(children: [
                Expanded(child: Text('Acumulado: $_totalAngle°', textAlign: TextAlign.center, style: TextStyle(fontSize: 10, color: cs.onSurfaceVariant))),
                if (_includeDirect) Expanded(child: Text('Directo: $_normalized°', textAlign: TextAlign.center, style: TextStyle(fontSize: 10, color: cs.onSurfaceVariant))),
              ]),
            ],
          ],
                    ...
                    : () async {
                        final ok = await showDialog<bool>(
                            ...
                            builder: (_) => AlertDialog(
                                    ...
                                    actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancelar')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Borrar'))]));
                        if (ok == true) {
          ...
          if (_history.isEmpty)
            Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(border: Border.all(color: cs.outlineVariant, style: BorderStyle.solid), borderRadius: BorderRadius.circular(12)),
                child: Text('Aún no hay elementos. La vista previa es automática; al guardar se creará el historial (Hive local, 100 máx).',
                    style: TextStyle(fontStyle: FontStyle.italic, color: cs.onSurfaceVariant, fontSize: 12), textAlign: TextAlign.center)),
          ..._history.map((e) => Card(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      ...
                      Container(
                          ...
                          child: Text('${e.operations.length} op · ${e.totalAngle}° → ${e.normalizedAngle}°', style: TextStyle(fontSize: 10, color: cs.onPrimaryContainer))),
                    ]),
                    const SizedBox(height: 4),
                    ...
                    Row(children: [
                      Expanded(
                        ...
                        const SizedBox(height: 4),
                        ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.file(File(e.imageOriginal), height: 80, fit: BoxFit.cover, errorBuilder: (_, _, _) => const Icon(Icons.image))),
                      ...
                      Expanded(
                        ...
                        const SizedBox(height: 4),
                        ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.file(File(e.imageResult), height: 80, fit: BoxFit.cover, errorBuilder: (_, _, _) => const Icon(Icons.image))),
